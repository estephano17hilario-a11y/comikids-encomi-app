...
def main():
    ...
    # 1. Configurar docker-compose.yml con evoapicloud/evolution-api:latest
...
networks:
  internal-network:
    external: true
EOF"""
    run_ssh_command(ssh, compose_content, "Actualizando a imagen latest")

    # 2. Levantar la nueva imagen
    run_ssh_command(ssh, "cd /opt/evolution && docker compose down && rm -rf /opt/evolution/instances/* && docker compose up -d", "Recreando contenedores con imagen latest")

    print("\n[*] Esperando 15 segundos para inicializacion de la API...")
    time.sleep(15)
    ...
    try:
        ...
        if b64:
            ...
            with open(output_path, "wb") as f:
                f.write(img_bytes)
            print(f"\n🎉 EXITO: QR GUARDADO EN: {output_path}")
        if pairing:
