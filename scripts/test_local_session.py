...
def main():
    ...
    # 1. Configurar CACHE_REDIS_ENABLED: "false" para usar multiFileAuthState en disco
...
networks:
  internal-network:
    external: true
EOF"""
    run_ssh_command(ssh, compose_content, "Actualizando docker-compose.yml a almacenamiento local de auth")

    # 2. Reiniciar Evolution API
    run_ssh_command(ssh, "cd /opt/evolution && docker compose stop evolution_api && rm -rf /opt/evolution/instances/* && docker compose up -d evolution_api", "Reiniciando Evolution API con disco local")

    print("\n[*] Esperando 12 segundos para inicio...")
    time.sleep(12)

    # 3. Eliminar y crear instancia
    run_ssh_command(ssh, 'curl -s -X DELETE "http://127.0.0.1:8080/instance/delete/comikids_whatsapp" -H "apikey: comikids_evolution_master_key_2026"', "Eliminando instancia vieja")
    time.sleep(2)
  ...
  -d '{
    ...
    _, create_res = run_ssh_command(ssh, create_cmd, "Creando instancia limpia comikids_whatsapp")
    ...
    # 4. Ver logs
    _, logs = run_ssh_command(ssh, "docker logs --tail 30 evolution_api", "Logs de Evolution API")

    # 5. Obtener QR
    _, connect_res = run_ssh_command(ssh, 'curl -s -X GET "http://127.0.0.1:8080/instance/connect/comikids_whatsapp" -H "apikey: comikids_evolution_master_key_2026"', "Obteniendo QR")
    ...
    try:
        ...
        if b64:
            ...
            with open(output_path, "wb") as f:
                f.write(img_bytes)
            print(f"\n🎉 EXITO TOTAL: QR GUARDADO EN {output_path}")
        else:
            print("CONNECT RES:", data)
